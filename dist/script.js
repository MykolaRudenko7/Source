...
window.addEventListener("DOMContentLoaded", function () {
  // модальні вікна
  Object(_modules_modals__WEBPACK_IMPORTED_MODULE_1__["default"])(); //   таби 1

  Object(_modules_tabs__WEBPACK_IMPORTED_MODULE_2__["default"])(".glazing_slider", ".glazing_block", ".glazing_content", "active"); //   таби 2

  Object(_modules_tabs__WEBPACK_IMPORTED_MODULE_2__["default"])(".decoration_slider", ".no_click", ".decoration_content > div > div", "after_click"); // форми
...
var forms = function forms() {
  // тут всі форми
  var allForms = document.querySelectorAll("form"); // тут всі інпути

  var inputs = document.querySelectorAll("input"); // тут всі повідомлення для користувача
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            ...
            document.querySelector(".status").textContent = message.loading; // в цій перемінній запрос і потім результат (норм чи не норм)
  ...
  allForms.forEach(function (form) {
    ...
    form.addEventListener("submit", function (e) {
      ...
      e.preventDafault(); // роблю блок з повідомленням
      // створюю дів

      var statusMessage = document.createElement("div"); // даю йому клас

      statusMessage.classList.add("status"); // поміщаю це повідомлення всередину форми

      form.appendChild(statusMessage); // збираю данні у вигляді формату FormData

      var formData = new FormData(form); // юрл і данні які я хочу відправить  на сервер

      postData("assets/server.php", formData) // якщо все добре
      ...
      .finally(function () {
        // очищую інпути
        clearInputs(); //  і видаляю повідомлення чере 5 сек
  ...
  function bindModal(selectorBtn, modalWindowSelector, closeSelector) {
    ...
    btns.forEach(function (btn) {
      ...
      close.addEventListener("click", function () {
        modal.style.display = "none"; // поки не закриєш, сайт не гортатиметься
...
var tabs = function tabs(headerSel, tabSel, contentSel, activeClass) {
  var header = document.querySelector(headerSel);
  var tabs = document.querySelectorAll(tabSel);
  var content = document.querySelectorAll(contentSel);
  var active = document.querySelector(activeClass); // функція для ховання контенту табу (для кожного елементу)
  ...
  function showTabContent() {
    ...
    content[i].style.display = "block"; //  і таб

    tabs[i].classList.add(activeClass);
  } // щоб правильно показать таби, спочатку ховаю їх


  hideTabContent(); // і потім показую

  showTabContent(); // при кліку на блок з табами

  header.addEventListener("click", function (e) {
    var target = e.target; //  якщо клік був на один з табів
    //  (рег вираз вирізає точку з класу (для classList))

    if (target && (target.classList.contains(tabSel.replace(/\./, "")) || //  можна з closest()
    target.parentNode.classList.contains(tabSel.replace(/\./, "")))) {
      ...
      tabs.forEach(function (tab, i) {
        ...
        if (target == tab || target.parentNode == tab) {
          // закриваю всі
          hideTabContent(); // показую який треба (на який клік)
